...
def leaves(t):
    ...
    if not t.branches:
        ...
        return [leaves(i)[0] for i in t.branches]
...
def deep_map(f, link):
    ...
    if isinstance(link.first, Link):
        flink = deep_map(f, link.first)
    else:
        flink = f(link.first)
    return Link(flink, deep_map(f, link.rest))
...
def reverse(link):
    ...
    """
    "*** YOUR CODE HERE ***"
    recent = Link(link.first)
    while link.rest != Link.empty:
        link = link.rest
        recent = Link(link.first, recent)
    return recent
