...
def insert(link, value, index):
    ...
    """
    "*** YOUR CODE HERE ***"
    list = [i for i in link]
    if index == 0:
        link.rest = list_to_link(list)
        link.first = value
    if index < len(list):
...
def computeAncesTree(t):
    ...
    "*** YOUR CODE HERE ***"
    if not t.is_leaf():
        firstp = [eth for eth in t.parents[0].ethnicity.keys()]
        secondp = [eth for eth in t.parents[1].ethnicity.keys()]

        p1_percent = [per for per in t.parents[0].ethnicity.values()]
        p2_percent = [per for per in t.parents[1].ethnicity.values()]


        firstdict = {eth : (t.parents[0].ethnicity[eth]/2.0) for eth in firstp}
        seconddict = {eth : (p2_percent[0]/2.0) for eth in secondp}


        for key, percent in firstdict.items():
            if key in seconddict:
                seconddict[key] = seconddict[key] + firstdict[key]
            else:
                seconddict[key] = percent

        for children in t.children.keys():
            children.ethnicity = seconddict

        morefams = [percent for percent in t.children.values()]
        for people in morefams:
            computeAncesTree(people)
